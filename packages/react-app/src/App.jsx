...
import { Alert, Button, Col, Menu, Row, Input, List, Divider } from "antd";
...
function App(props) {
  ...
  const [ethStakeValue, setEthStakeValue] = useState(0.0001);
  ...
  // ** keep track of a user state in the contract:
  const userState = useContractReader(readContracts, "Staker", "addressToUser", [address]);
  ...
  const claimDeadline = useContractReader(readContracts, "Staker", "claimDeadline");
  const withdrawalDeadline = useContractReader(readContracts, "Staker", "withdrawalDeadline");
  ...
  if (complete) {
    completeDisplay = (
      <div style={{padding: 64, backgroundColor: "#eeffef", fontWeight: "bold", color: "rgba(0, 0, 0, 0.85)" }} >
        ...
        <Balance balance={exampleExternalContractBalance} fontSize={32} /> total ETH locked!
      </div>
    );
  }
        ...
        <Switch>
          <Route exact path="/">
            ...
            <div style={{ padding: 8, marginTop: 16, fontWeight: "bold" }}>
              ...
              { claimDeadline && <Countdown value={claimDeadline.toNumber() * 1000} /> }
            </div>

            <div style={{ padding: 8, marginTop: 16, fontWeight: "bold"}}>
              <div>Withdrawal Period Left:</div>
              { withdrawalDeadline && <Countdown value={withdrawalDeadline.toNumber() * 1000} /> }
            </div>
            ...
            <div style={{ padding: 8,fontWeight: "bold" }}>
              ...
              <Balance balance={userState ? userState.balance : 0} fontSize={64} />
            </div>
            <div style={{ padding: 8 }}>
              <Button
                type={"default"}
            ...
            <div style={{ padding: 8 }}>
              <Input
                ...
                min="0.0001"
                value={ethStakeValue}
                onChange={(e) => setEthStakeValue(e.target.value)}
              />
              <Button
                type={(userState ? userState.balance : 0) ? "success" : "primary"}
                onClick={() => {
                  tx(writeContracts.Staker.stake({ value: ethers.utils.parseEther(ethStakeValue.toString()) }));
                }}
                disabled={!ethStakeValue}
              >
                🥩 Stake ether!
              </Button>
            </div>
